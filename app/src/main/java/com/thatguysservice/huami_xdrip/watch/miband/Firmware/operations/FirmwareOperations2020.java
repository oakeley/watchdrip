...
public class FirmwareOperations2020 extends FirmwareOperationsNew {
    ...
    public FirmwareOperations2020(byte[] file, SequenceState sequenceState, MiBandService service) {
        ...
        switch (seq) {
            case SequenceStateVergeNew.REQUEST_PARAMETERS: {
                ...
                connection.writeCharacteristic(getFirmwareCharacteristicUUID(), getRequestParametersCommand())
                        .subscribe(val -> {
                                    if (d)
                                        UserError.Log.d(TAG, "Wrote getRequestParametersCommand: " + Helper.bytesToHex(val));
                                },
                                throwable -> {
